...
class BasicTests(unittest.TestCase):
    ...
    def test_login_logout(self, client):
        ...
        rv = self.login(client, f"{email}x", password)
        assert b'Check your Login Credentials and Try Again' in rv.data

        rv = self.login(client, email, f'{password}x')
        assert b'Check your Login Credentials and Try Again' in rv.data

    def test_profile_for_loggedin(self, client):
        """Make sure Profile is Visible if user is logged In"""

        email = self.app.config["EMAIL"]
        password = self.app.config["PASSWORD"]

        _ = self.login(client, email, password)

        rv = self.profile_page(client)
        assert b'Welcome, to Your Profile' in rv.data

    def test_profile_for_loggedout(self, client):
    ...
    def test_posts_for_loggedin(self, client):
        ...
        rv = self.posts_page(client)
        assert b'Welcome To Posts Page' in rv.data

    def test_posts_for_loggedout(self, client):
